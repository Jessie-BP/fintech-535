...
def candlestick_figure(df_stock: pd.DataFrame, ticker: str) -> go.Figure:
    ...
    fig.update_layout(
        ...
        annotations=[
            dict(
                text="Underlying close uses TRDPRC_1; option marks use MID_PRICE",
                xref="paper",
                yref="paper",
                x=0.0,
                y=1.02,
...
def price_surface_figure(
    ...
    fig.update_layout(
        ...
        annotations=[
            dict(
                text="Cyan = closing NBBO MID_PRICE &nbsp;·&nbsp; Magenta = last trade (TRDPRC_1) &nbsp;·&nbsp; Sheet is interpolated, not a market",
                xref="paper",
                yref="paper",
                x=0.0,
                y=1.0,
...
def mid_vs_trade_figure(wide: pd.DataFrame, asof=None, ticker: str = "UUUU") -> go.Figure:
    ...
    fig.update_layout(
        ...
        font=dict(color="#e6edf3", family="Inter, system-ui, sans-serif", size=12),
        title=dict(
            text=f"{ticker}  ·  last trade is not the closing midpoint",
            font=dict(size=16, family="Inter, system-ui, sans-serif"),
            x=0.02,
            xanchor="left",
            y=0.98,
