...
def flatten_lseg_options(df_options: pd.DataFrame) -> pd.DataFrame:
    ...
    if isinstance(cols, pd.MultiIndex):
        ...
        for i, vals in enumerate(level_values):
            ...
            if upper & {"TRDPRC_1", "MID_PRICE", "CLOSE", "BID", "ASK"}:
                field_level = i
                break
...
def surface_grid(points: pd.DataFrame, value_col: str, n_strike: int = 40, n_dte: int = 30):
    ...
    # A 2D triangulation is impossible when all observations share one strike
    # or one expiry. The point cloud is still valid; only omit the sheet.
...
def summarize_sparsity(wide: pd.DataFrame) -> dict:
    """Classroom-facing counts that make the midpoint ≠ last-trade point."""
...
def synthesize_demo_payload(
    ...
    def ric_for(expiry: dt.date, strike: float, cp: str) -> str:
        call_code = chr(ord("A") + expiry.month - 1)
        month_code = call_code if cp == "C" else chr(ord("M") + expiry.month - 1)
        strike_str = f"{int(round(strike * 100)):05d}"
        base = f"{ticker_root}{month_code}{expiry.strftime('%d')}{expiry.strftime('%y')}{strike_str}.U"
        return f"{base}^{call_code}{expiry.strftime('%y')}"
        ...
        for strike in strikes:
            for cp in ("C", "P"):
                ...
                for ts, spot in df_stock["TRDPRC_1"].items():
                    ...
                    mid = max(intrinsic + time_val, 0.01)
                    # Closing midpoint is a mark; last trade is noisier / stickier.
                    trade = mid * rng.normal(1.0, 0.06) + rng.normal(0, 0.02)
                    trade = max(trade, 0.01)

                    # Liquidity mask: a midpoint usually exists near the money;
                    # prints only when DTE is short or the strike is close to spot.
                    near_money = mny < 0.25
                    listed = mny < 0.55 or dte < 21
                    if not listed:
                        continue
                    records[(ric, "MID_PRICE")].loc[ts] = round(mid, 4)
