...
2. MID_PRICE (closing NBBO midpoint) is not TRDPRC_1 (last trade).
...
def load_or_fetch_pipeline_data(
    ...
    if os.path.exists(CACHE_FILE):
        ...
        option_columns = payload.get("options", pd.DataFrame()).columns
        fields = (
            {str(v).upper() for level in range(option_columns.nlevels)
             for v in option_columns.get_level_values(level)}
            if isinstance(option_columns, pd.MultiIndex)
            else set()
        )
        if "MID_PRICE" in fields:
            return payload
        if payload.get("synthetic"):
            print("Old synthetic cache uses SETTLE; rebuilding it with MID_PRICE.")
            return synthesize_demo_payload(ticker_root=ticker_root, ticker_stock=ticker_stock)
        raise ValueError(
            f"{CACHE_FILE} is a stale cache without MID_PRICE. Move it aside and "
            "run again with an authenticated LSEG session to fetch MID_PRICE."
        )
    ...
    for d in friday_dates:
        ...
        for strike in strikes:
            ...
            # For an expired put, the body uses M-X but the suffix always uses
            # the corresponding call-month code A-L (for example T...^H26).
...
class State(rx.State):
    ...
    def load_data(self):
        ...
        expiry_counts = wide.groupby("date")["expiry"].nunique() if len(wide) else pd.Series(dtype=int)
        richest_dates = expiry_counts[expiry_counts == expiry_counts.max()].index if len(expiry_counts) else []
        self.asof = (
            max(richest_dates).strftime("%Y-%m-%d")
            if len(richest_dates)
...
def index() -> rx.Component:
    return rx.container(
        rx.vstack(
            ...
            rx.text(
                "Cyan dots = closing NBBO midpoint (MID_PRICE). Magenta diamonds = last trade. "
            ...
            rx.card(
                rx.vstack(
                    rx.heading("What the sparse surface tells us", size="4"),
                    rx.text(
                        "Price observations are densest around near-the-money strikes "
                        "and shorter expiries, while the wings and longer maturities "
                        "contain many empty cells."
                    ),
                    rx.text(
                        "Interpolating across empty cells on a $0.50 strike grid is "
                        "dangerous because it creates apparently precise prices for "
                        "contracts that may never have been quoted or traded."
                    ),
                    rx.text(
                        "Next week I will treat MID_PRICE as the mark of the close and TRDPRC_1 "
                        "as evidence that an actual trade occurred."
                    ),
                    spacing="3",
                    align="start",
                    ),
