...
| Due | Monday, Sep 07 at 11:59pm (per the later Canvas announcement) |
...
| Submission | Publish the app with GitHub Pages and submit the site link on Canvas. |
...
- On a strike that has a midpoint and no print, what price would you actually 
...
- `MID_PRICE` — the closing NBBO midpoint, `(bid + ask) / 2` at the exchange close. LSEG does not expose a true exchange settlement price for this expired US equity-option RIC space. A midpoint may still come from a stale quote.
...
- Translucent sheet: linear interpolation of the midpoints. That sheet is **not** the market. Toggle it off.
...
| `^{M}{YY}` | Expired-contract suffix; uses the corresponding call-month code `A–L` and year, even for puts |
...
- Invert Black–Scholes on the midpoints to get a crude IV surface. Use a constant rate. Write down what you assumed. Next week: that IV still is not the price you can trade.
...
- Do not use `CLOSE`, `MID_PRICE`, and `TRDPRC_1` as synonyms. This pull requests `TRDPRC_1` and `MID_PRICE` only.
